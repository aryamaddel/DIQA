...
def extract_features(preprocessed_data: dict) -> np.ndarray:
    ...
    features.extend([
        ...
        float(np.mean(np.sqrt(grad_x**2 + grad_y**2)))
    ])
    ...
    blockiness = np.abs(np.mean(block_variances) - np.mean(neighbor_variances)) if block_variances else 0.0
    ...
    if center_size >= 8:
        ...
        mid_energy = np.mean(dct_abs[third:2*third, third:2*third]) if 2*third <= size else 0.0
        high_energy = np.mean(dct_abs[2*third:, 2*third:]) if 2*third < size else 0.0
