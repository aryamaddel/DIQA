...
if "AllMOS_release" in mos_data:
    ...
    mos_scores = list(mos_data.values())[
        3
    ].flatten()  # Skip __header__, __version__, __globals__
...
print(
    f"Normalized MOS range: [{live_mos_df['MOS'].min():.2f}, {live_mos_df['MOS'].max():.2f}]"
)
...
print(
    f"Tested on: LIVE In-the-Wild ({len(predictions)} images, normalized to 0-5 scale)"
)
...
pd.DataFrame(
    {
        ...
        "ground_truth_original_0_100": live_mos_df.loc[
            valid_indices, "MOS_original"
        ].values,
